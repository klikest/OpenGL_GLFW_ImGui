...
void Grid3D::grid_dexel_draw()
{
    ...
    for (int i = 0; i < dexel_tool.size(); i++)
    {
        grid_draw.push_back(glm::vec4(i % X_tool_size - X_tool_size / 2, (i / X_tool_size) % Y_tool_size - Y_tool_size / 2, dexel_tool[i].x, dexel_tool[i].y));
    }
}
